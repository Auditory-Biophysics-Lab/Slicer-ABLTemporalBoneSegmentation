...
class SkullThicknessMappingWidget(ScriptedLoadableModuleWidget):
    ...
    def setup(self):
        ...
        SkullThicknessMappingLogic.process("1601L")
...
class SkullThicknessMappingLogic(ScriptedLoadableModuleLogic):
    ...
    @staticmethod
    def process(sample_name, parameters=None):
        parameters = {
          'n_dura_voxel': 20,
          'thresh_bone': 600,
          'thresh_aircell': 150,
        ...
        uniD = 10
        data = {
          'image_name': SkullThicknessMappingLogic.sample_folder() + 'Images/' + sample_name + '.hdr',
          'outer_stl_data': SkullThicknessMappingLogic.read_stl(SkullThicknessMappingLogic.sample_folder() + 'STL/' + sample_name + '_outer.stl'),
          # 'rest_stl_data': SkullThicknessMappingLogic.read_stl(SkullThicknessMappingLogic.sample_folder() + 'STL/' + sample_name + '_rest.stl')
        }
        # print(data)
        # for each
    ...
    def read_stl(file_name):
        ...
        with open(file_name, mode='r') as file:
            ...
            for lines in itertools.izip_longest(*[iter(file)] * 7, fillvalue=None):
                assert len(lines) == 7
                print("facet " + str(len(data)+1) + ": " + str(lines))
                data.append(Facet(
                    n=[float(s) for s in re.findall(r"[-+]?\d*\.\d+|\d+", lines[1])],
                    v1=[float(s) for s in re.findall(r"[-+]?\d*\.\d+|\d+", lines[3])],
                    v2=[float(s) for s in re.findall(r"[-+]?\d*\.\d+|\d+", lines[4])],
                    v3=[float(s) for s in re.findall(r"[-+]?\d*\.\d+|\d+", lines[5])]
                ))
